(function() {
  'use strict';
  var CP, GUY, NOTIFIER, alert, debug, echo, help, info, inspect, log, plain, praise, rpr, rvr, urge, warn, whisper;
  ...
  ({alert, debug, help, info, plain, praise, urge, warn, whisper} = GUY.trm.get_loggers('MEMORY-USAGE-ALERTER'));
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.notify_warn = function(message) {
    var settings;
    ...
    return NOTIFIER.notify(settings);
  };

  //-----------------------------------------------------------------------------------------------------------
  this.sync = function() {
    process.setuid('root');
    return debug(CP.execSync('echo 3 >> /proc/sys/vm/drop_caches', {
      encoding: 'utf-8'
    }));
  };
...
//# sourceMappingURL=memory-usage-alerter.js.map