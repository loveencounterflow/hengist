(function() {
  ...
  add_views = function(db) {
    ...
    db(SQL`drop view if exists dbay_db_overview;
create view dbay_db_overview as select
  ...
  where true
    ...
    ;`);
    db(SQL`drop view if exists dbay_fk_view_1;
create view dbay_fk_view_1 as select
  ...
  order by schema, from_table_name, fk_id, fk_idx;`);
    db(SQL`drop view if exists dbay_fk_view_2;
...
//# sourceMappingURL=suspected-bug-in-sqlite-json_group_array.js.map