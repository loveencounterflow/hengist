(function() {
  'use strict';
  var CND, Dba, PATH, Readlines, Scdadba, badge, debug, echo, freeze, glob, help, info, lets, rpr, urge, warn, whisper;
...
create table ${schema_i}.lines (
  ...
  primary key ( short_path, lnr ) );`);
    ...
    $add_line(cfg) {
      ...
      return this.run(`insert into ${this.cfg.schema_i}.lines ( short_path, lnr, line )
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.patterns = [['method', /^\s+(?<atsign>@?)(?<name>\w+):(?<tail>.*)$/]];
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.demo = function() {
    var dba, groups, i, j, len, len1, line, lnr, match, path, pattern, prefix, readlines, ref, ref1, ref2, row, schema, short_path, source_glob, source_paths, tag;
    schema = 'scda';
...
//.........................................................................................................
    for (i = 0, len = source_paths.length; i < len; i++) {
      ...
      while ((line = readlines.next()) !== false) {
        ...
        for (j = 0, len1 = ref.length; j < len1; j++) {
          ...
          groups = this._groups_from_match(match);
    ...
    ref2 = dba.query(`select * from scda.lines
where true
