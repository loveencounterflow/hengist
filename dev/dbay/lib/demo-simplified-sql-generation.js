...
create view dbay_create_table_clauses as select
  ...
  from dbay_tables;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists dbay_drop_table_clauses;
create view dbay_drop_table_clauses as select
  ...
  from dbay_tables;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists dbay_create_table_statements_1;
create view dbay_create_table_statements_1 as
  ...
  order by section_nr, table_nr, part_nr, lnr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists dbay_create_table_statements_2;
create view dbay_create_table_statements_2 as select
  ...
  order by section_nr, table_nr, part_nr, r1.lnr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists dbay_create_table_statements_3;
create view dbay_create_table_statements_3 as select
  ...
  order by lnr, tail;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists dbay_create_table_statements_4;
create view dbay_create_table_statements_4 as select
  ...
  order by lnr, tail;`);
    ...
    db.create_table_function({
      ...
      columns: ['vnr2', 'txt'],
      rows: function*(line, nxt_line) {
        // debug [ line, nxt_line, ]
        if (skip) {
          skip = false;
        } else if (nxt_line.startsWith('\b')) {
          skip = true;
          yield [1, line + nxt_line.slice(1)];
        } else {
          yield [1, line];
        }
        return null;
...
create view ab as select
  ...
  order by x421;`);
    ...
    txt = ((function() {
      var ref1, results;
      ref1 = db(SQL`select * from dbay_create_table_statements`);
      results = [];
      for (row of ref1) {
