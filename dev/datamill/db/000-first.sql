...
create function DEMO.expand_vnr_for_sorting( ¶vnr integer[] )
  returns integer[] immutable parallel safe language plpgsql as $$
  declare
  ...
  begin
    ...
    return array_cat( ¶vnr, array_fill( 0, array[ ¶max_length - ¶length ] ) );
    end; $$;

create function DEMO.expand_vnr_for_sorting( ¶vnr float8[] )
  returns float8[] immutable parallel safe language sql as $$
  select DEMO.expand_vnr_for_sorting( ¶vnr::integer[] )::float8[]; $$;
...
create function DEMO.add_final_zero( ¶vnr integer[] )
  ...
  --   ¶length     integer := array_length( ¶vnr, 1 );
  begin
    return array_append( ¶vnr, 0 );
    end; $$;

create function DEMO.add_final_zero( ¶vnr float8[] )
  returns float8[] immutable parallel safe language sql as $$
  select DEMO.add_final_zero( ¶vnr::integer[] )::float8[]; $$;
