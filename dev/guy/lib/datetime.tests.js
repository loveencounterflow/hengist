(function() {
  ...
  ({alert, debug, help, info, plain, praise, urge, warn, whisper} = GUY.trm.get_loggers('GUY/datetime'));
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.guy_datetime_api = function(T, done) {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._demo_datetime = function(cfg) {
    var D, d;
    GUY = require(H.guy_path);
    D = GUY.datetime;
    //---------------------------------------------------------------------------------------------------------
    ...
    debug('^34534-10^', D.now({
      add: [1, 'week']
    }));
    debug('^34534-11^', D.now({
      subtract: [1, 'week'],
      add: [1, 'week']
    }));
    debug('^34534-9^', D.isots_from_srts(D.now({
      subtract: [1, 'month']
    })));
    return null;
  };
  ...
  if (require.main === module) {
    (() => {
      ...
      // test @
      return this._demo_datetime();
    })();
  }

}).call(this);

//# sourceMappingURL=datetime.tests.js.map