(function() {
  'use strict';
  var CND, DATOM, alert, badge, cast, debug, demo, echo, first_of, help, info, isa, jr, last_of, lets, log, new_datom, rpr, select, test, type_of, types, urge, validate, warn, whisper;
  ...
  ({isa, validate, cast, first_of, last_of, type_of} = types);
  ...
  // TESTS
  //-----------------------------------------------------------------------------------------------------------
  this["INTERTEXT.WRAP.justify_monospaced"] = async function(T, done) {
    ...
    probes_and_matchers = [
      ...
      [
        ['just another day in a bonkers place',
        50],
        {
          blanks: 21,
          solids: 29
    ...
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ...
      await T.perform(probe, matcher, error, function() {
        return new Promise(function(resolve, reject) {
          var blanks, line_width, result, solids, stats, text, words;
          [text, line_width] = probe;
          words = text.split(/\s+/);
...
//# sourceMappingURL=wordwrap.test.js.map