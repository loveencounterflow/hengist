(function() {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this["path union 1"] = function(T, done) {
    var Svgttf2, path, result, svg, svgttf2;
    ...
    if (T != null) {
      T.eq(result, {
        ...
        shy: {
          ...
          pd: 'M-204,-115h178v-802h50v802h178v70h-406z'
        }
      });
    }
  ...
  //-----------------------------------------------------------------------------------------------------------
  this["path union 2"] = function(T, done) {
    var Svgttf2, path, result, svg, svgttf2;
    ({Svgttf2} = require('../../../apps/svgttf/lib/svgttf-next'));
    svgttf2 = new Svgttf2();
  ...
  //-----------------------------------------------------------------------------------------------------------
  this["intersection between paths"] = function(T, done) {
    var PAPER, baseguide, baseline, intersection, out_1_svg, out_2_svg, out_fspath_1, out_fspath_2, project, rectangle, x1, x2, y1, y2;
    ...
    FS.writeFileSync(out_fspath_1, out_1_svg = project.exportSVG({
      asString: true,
      precision: 1
    }));
    intersection = baseguide.getIntersections(rectangle);
    baseline = new PAPER.Path.Line({
      from: intersection[0].point,
      to: intersection[1].point,
      strokeColor: '#0f0',
      strokeWidth: 3
    });
    FS.writeFileSync(out_fspath_2, out_2_svg = project.exportSVG({
      asString: true,
      precision: 1
    }));
  ...
  //-----------------------------------------------------------------------------------------------------------
  this["baselines from intersection of column and baselinegrid"] = function(T, done) {
    var Svgttf2, out_path, out_svg, path, pd, ref, result, svg, svgttf2;
    ({Svgttf2} = require('../../../apps/svgttf/lib/svgttf-next'));
    svgttf2 = new Svgttf2();
    ...
    result = svgttf2.glyf_pathdata_from_svg({path, svg});
    debug('^34234^', result);
    pd = (ref = result['ooo']) != null ? ref.pd : void 0;
...
<path stroke='green' stroke-width='1' d='${pd}'/>
  ...
  if (module.parent == null) {
    ...
    this["intersection between paths"]();
  }

}).call(this);

//# sourceMappingURL=svgttf-next.tests.js.map