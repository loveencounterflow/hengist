...
def new_segment( lohi: gen_segment = None, *, lo: int = None, hi: int = None ) -> Segment:
  ...
  if lohi is None:
    if type( lo ) != int: raise InterlapValueError( f"^E342^ illegal arguments: lohi: {lohi}, lo: {lo}, hi: {hi}" )
    if type( hi ) != int: raise InterlapValueError( f"^E345^ illegal arguments: lohi: {lohi}, lo: {lo}, hi: {hi}" )
  else:
