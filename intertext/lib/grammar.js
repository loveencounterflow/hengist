(function() {
  'use strict';
  var CHVTN, CND, DATOM, Grammar, INTERTEXT, MAIN, Multimix, alert, assign, badge, datom_from_token, debug, echo, freeze, help, info, is_given, isa, jr, lets, log, new_grammar, new_ref, new_token, rpr, set_class_name, type_of, types, urge, validate, vnr_from_parser_error, warn, whisper;
  ...
  vnr_from_parser_error = function(error) {
    var R, col_nr, i, key, len, line_nr, ref1, token;
    R = null;
    ...
    for (i = 0, len = ref1.length; i < len; i++) {
      ...
      if ((is_given(line_nr = token.startLine)) && (is_given(col_nr = token.startColumn))) {
        R = [line_nr, col_nr];
        break;
      }
    }
